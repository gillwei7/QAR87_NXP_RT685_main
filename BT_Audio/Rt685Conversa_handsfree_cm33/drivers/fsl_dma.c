...
unsigned int *Ptr_dma_descriptor_table0=(unsigned int *)s_dma_descriptor_table0;	//zzz --- for checking dual buffer, who is being used
...
void DMA_StartTransfer(dma_handle_t *handle)
{
    ...
    if ((handle->base->CHANNEL[handle->channel].CFG & DMA_CHANNEL_CFG_HWTRIGEN_MASK) == 0U)
    {
    	...
    	//handle->base->CHANNEL[channel].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;		//zzz deleted --- transfer start control is moved outside
    }
}
void DMA_StartTransfer_NoStartImmediately(dma_handle_t *handle)
{
    assert(NULL != handle);
    assert(FSL_FEATURE_DMA_NUMBER_OF_CHANNELSn(handle->base) != -1);
