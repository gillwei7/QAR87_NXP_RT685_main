...
//---------------var for PDM0,1,2,3,4,5 DMA IO------------------ beg------
	...
	volatile S32 MicInputDmaDualBuf_0[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_1[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
#endif
#if EnableMic23==1
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_2[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_3[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
#endif
#if EnableMic45==1
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_4[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_5[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
#endif
#if EnableMic67==1
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_6[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
	__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(4)))
	volatile S32 MicInputDmaDualBuf_7[AudioFrameSizeInSamplePerCh * PDM_Rx_BUFFER_NUM];
#endif
...
__attribute__((__section__(".data.$Audio_io_data_non_cached"))) __attribute__((aligned(16)))
	...
	dmic_transfer_t MicDmaCfgCh0[2U] = {
		/* transfer configurations for channel0 */
		{
			...
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
			.linkTransfer           = &MicDmaCfgCh0[1],
		},

		{
			.data                   = (void *)&MicInputDmaDualBuf_0[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh1[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_1[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh2[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_2[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh3[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_3[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh4[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_4[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh5[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_5[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh6[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_6[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
	...
	dmic_transfer_t MicDmaCfgCh7[2U] = {
		...
		{
			.data                   = (void *)&MicInputDmaDualBuf_7[AudioFrameSizeInSamplePerCh],
			.dataWidth              = sizeof(S32),
			.dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32),
			.dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth,
...
void ImmediatelyStartDmicDmaChannels(U8 MicEnableBits)	//bit0~7 --> dmic ch0~7
{
#if 1
...
#else
	#if EnableMic01==1
		if(MicEnableBits&0x01)
		{
			DMIC_DoFifoReset(DMIC0, 0);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC0].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
		if(MicEnableBits&0x02)
		{
			DMIC_DoFifoReset(DMIC0, 1);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC1].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
	#endif
	#if EnableMic23==1
		if(MicEnableBits&0x04)
		{
			DMIC_DoFifoReset(DMIC0, 2);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC2].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
		if(MicEnableBits&0x08)
		{
			DMIC_DoFifoReset(DMIC0, 3);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC3].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
	#endif
	#if EnableMic45==1
		if(MicEnableBits&0x10)
		{
			DMIC_DoFifoReset(DMIC0, 4);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC4].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
		if(MicEnableBits&0x20)
		{
			DMIC_DoFifoReset(DMIC0, 5);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC5].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
	#endif
	#if EnableMic67==1
		if(MicEnableBits&0x40)
		{
			DMIC_DoFifoReset(DMIC0, 6);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC6].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
		if(MicEnableBits&0x80)
		{
			DMIC_DoFifoReset(DMIC0, 7);
			((DMA_Type *)(DEMO_DMA))->CHANNEL[DMAREQ_DMIC7].XFERCFG |= DMA_CHANNEL_XFERCFG_SWTRIG_MASK;
		}
...
void Init_MicDmaCfgCh(U8 MicEnableBits)
{
	#if EnableMic01==1
		if(MicEnableBits&0x01)
		{
			...
			MicDmaCfgCh0[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh0[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh0[0].linkTransfer           = &MicDmaCfgCh0[1];

			MicDmaCfgCh0[1].data                   = (void *)&MicInputDmaDualBuf_0[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh0[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh0[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x02)
		{
			...
			MicDmaCfgCh1[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh1[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh1[0].linkTransfer           = &MicDmaCfgCh1[1];

			MicDmaCfgCh1[1].data                   = (void *)&MicInputDmaDualBuf_1[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh1[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh1[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x04)
		{
			...
			MicDmaCfgCh2[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh2[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh2[0].linkTransfer           = &MicDmaCfgCh2[1];

			MicDmaCfgCh2[1].data                   = (void *)&MicInputDmaDualBuf_2[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh2[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh2[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x08)
		{
			...
			MicDmaCfgCh3[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh3[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh3[0].linkTransfer           = &MicDmaCfgCh3[1];

			MicDmaCfgCh3[1].data                   = (void *)&MicInputDmaDualBuf_3[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh3[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh3[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x10)
		{
			...
			MicDmaCfgCh4[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh4[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh4[0].linkTransfer           = &MicDmaCfgCh4[1];

			MicDmaCfgCh4[1].data                   = (void *)&MicInputDmaDualBuf_4[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh4[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh4[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x20)
		{
			...
			MicDmaCfgCh5[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh5[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh5[0].linkTransfer           = &MicDmaCfgCh5[1];

			MicDmaCfgCh5[1].data                   = (void *)&MicInputDmaDualBuf_5[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh5[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh5[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x40)
		{
			...
			MicDmaCfgCh6[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh6[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh6[0].linkTransfer           = &MicDmaCfgCh6[1];

			MicDmaCfgCh6[1].data                   = (void *)&MicInputDmaDualBuf_6[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh6[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh6[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
		...
		if(MicEnableBits&0x80)
		{
			...
			MicDmaCfgCh7[0].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
			MicDmaCfgCh7[0].dataAddrInterleaveSize = kDMA_AddressInterleave1xWidth;
			MicDmaCfgCh7[0].linkTransfer           = &MicDmaCfgCh7[1];

			MicDmaCfgCh7[1].data                   = (void *)&MicInputDmaDualBuf_7[AudioFrameSizeInSamplePerCh];
			MicDmaCfgCh7[1].dataWidth              = sizeof(S32);
			MicDmaCfgCh7[1].dataSize               = AudioFrameSizeInSamplePerCh*sizeof(S32);
...
#if 1	//folding --- init
void BOARD_DeInit_DMA_PDM(U8 MicEnableBits)
{
...
void BOARD_Init_DMA_PDM(U8 MicEnableBits)
{
	#if EnableMic01==1
		if(MicEnableBits&0x01)
		{
...
void BOARD_Init_DMIC(U8 MicEnableBits, U8 SkipInitGlobalDMIC0)
{
...
{
	DbgPin7Up();
	StartI2SAudioDmaFromDmicDmaIntr_Cnt++;
	if(StartI2SAudioDmaFromDmicDmaIntr_Cnt>2)
	{
...
void ConfigDmicChainedDma(U8 MicEnableBits)
{
	#if EnableMic01==1
		if(MicEnableBits&0x01)
		{
