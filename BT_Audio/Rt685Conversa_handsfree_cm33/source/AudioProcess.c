...
volatile int *MicInputCh0Ptr;
volatile int *MicInputCh1Ptr;
volatile int *MicInputCh4Ptr;
volatile int *MicInputCh5Ptr;
...
#if 1	//folding
const S16 Table_GenerateSinWav_S16[20+128]=    //cycle count number is 20
{
	     0.00000000f*32768,   0.16030884f*32768,    0.30493164f*32768,    0.41967773f*32768,    0.49337769f*32768,
	     0.51876831f*32768,   0.49337769f*32768,    0.41967773f*32768,    0.30493164f*32768,    0.16030884f*32768,
	     0.00000000f*32768,  -0.16030884f*32768,   -0.30493164f*32768,   -0.41967773f*32768,   -0.49337769f*32768,
	    -0.51876831f*32768,  -0.49337769f*32768,   -0.41967773f*32768,   -0.30493164f*32768,   -0.16030884f*32768,
	    ...
	    -0.51876831f*32768,  -0.49337769f*32768,   -0.41967773f*32768,   -0.30493164f*32768,   -0.16030884f*32768,
	     ...
	     0.00000000f*32768,   0.16030884f*32768,    0.30493164f*32768,    0.41967773f*32768,    0.49337769f*32768,
	     0.51876831f*32768,   0.49337769f*32768,    0.41967773f*32768
};
S16   *GenerateSinWav_Ptr_S16=(S16 *)  (Table_GenerateSinWav_S16+10);
float Gain_GenerateSinWav_L=0.81f;
float Gain_GenerateSinWav_R=0.82f;

void GenerateSinWav_S32(S32 *DstPtrL,S32 *DstPtrR, int L)	//L should be <=128
{
    U16 i;
    for(i=0;i<L;i++)
    {
    ...
    while(GenerateSinWav_Ptr_S16>=(Table_GenerateSinWav_S16+20))
        GenerateSinWav_Ptr_S16-=20;
}
void GenerateSinWav_S16_LRMixed(S16 *DstPtr, int L)			//L should be <=128
{
    U16 i;
    for(i=0;i<L;i++)
    {
...
void GenerateSinWav_S16_SingleCh(S16 *DstPtr, int L)		//L should be <=128
{
    U16 i;
    for(i=0;i<L;i++)
    {
