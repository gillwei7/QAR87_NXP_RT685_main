...
static usb_device_class_config_struct_t g_CompositeClassConfig[4] = {{
														...
														#ifdef EnableMidiInTheDemo
																	{
																		USB_DeviceAudioCompositeMidiCallback,
																		(class_handle_t)NULL,
																		&g_UsbDeviceAudioClassMidi,
																	}
														#endif
...
void SCTIMER_SOF_TOGGLE_HANDLER()
{
    ...
    if (g_composite.audioUnified.attach)
    {
        if (abs(pllCount - AUDIO_PLL_USB1_SOF_INTERVAL_COUNT) < (AUDIO_PLL_USB1_SOF_INTERVAL_COUNT >> 7))
        {
            ...
            {
                if (AODOfUacDnBuf > ((UacDnBuf_LengthInMs *3/4) * AUDIO_OUT_SAMPLING_RATE_KHZ))
                {
                    g_composite.audioUnified.curAudioPllFrac++;
                    pll_change = 1;
                    PRINTF("+\n");
                }
                else if (AODOfUacDnBuf <= ((UacDnBuf_LengthInMs/4) * AUDIO_OUT_SAMPLING_RATE_KHZ))
                {
                    g_composite.audioUnified.curAudioPllFrac--;
                    pll_change = 1;
                    PRINTF("-\n");
    ...
    switch (event)
    {
        ...
        case kUSB_DeviceEventSetConfiguration:
            ...
            else if (USB_COMPOSITE_CONFIGURE_INDEX == (*temp8))
            {
				...
				#ifdef EnableMidiInTheDemo
					USB_DeviceAudioMidiCompositeSetConfigure(g_composite.midiDev.midiHandle, *temp8);
				#endif
        ...
        case kUSB_DeviceEventSetInterface:
            ...
            if (g_composite.attach)
            {
				...
				#ifdef EnableMidiInTheDemo
					else if (USB_AUDIO_MIDI_STREAM_INTERFACE_INDEX == interface)
					{
						error = USB_DeviceAudioMidiSetInterface(g_composite.midiDev.midiHandle, interface, alternateSetting);
					}
				#endif
                else
                {
...
void UsbAppInit(void)
{
	...
	#ifdef EnableMidiInTheDemo
		g_composite.midiDev.midiHandle               = (class_handle_t)NULL;
	#endif
    ...
    else
    {
		...
		#ifdef EnableMidiInTheDemo
			g_composite.midiDev.midiHandle               = g_UsbDeviceCompositeConfigList.config[3].classHandle;
		#endif
		...
		#ifdef EnableMidiInTheDemo
			USB_DeviceMidiInit          (&g_composite);
		#endif
