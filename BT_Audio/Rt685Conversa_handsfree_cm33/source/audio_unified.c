...
void U SB_AudioFeedbackDataUpdate()
{
...
void USB_MicUpStreamDataRateControl_AdjustPacketLength(int AodInCirBuf)//amount of free space
{
    ...
    U16 HiLimitInSample, LoLimitInSample;
		...
		if(MicUpStreamDataRateAdjustCnt++%120)
		{
		...
		if(MicUpStreamDataRateAdjustCnt++%60)
		{
		    UsbAudioUpstreamLengthAdjusted=PacketSize;
			return;
		}
		...
		if(MicUpStreamDataRateAdjustCnt++%30)
		{
		    UsbAudioUpstreamLengthAdjusted=PacketSize;
			return;
		}
		...
		if(MicUpStreamDataRateAdjustCnt++%15)
		{
		    UsbAudioUpstreamLengthAdjusted=PacketSize;
			return;
		}
    ...
    switch (event)
    {
        ...
        case kUSB_DeviceAudioEventStreamRecvResponse:
            ...
            if ((PtrUsbDevComposite->audioUnified.attach) && (ep_cb_param->length != (USB_UNINITIALIZED_VAL_32)))
            {
				...
				//if ((CirUacDnAudioBuf_SpaceOccupiedInSamples_MultiCh(&UacDnAudioBuf_MCh) >= (UacDnBuf_LengthInMs*AUDIO_OUT_SAMPLING_RATE_KHZ-AudioFrameSizeInSamplePerCh)/2+AUDIO_OUT_SAMPLING_RATE_KHZ) && (PtrUsbDevComposite->audioUnified.startPlayHalfFull == 0))
				{
