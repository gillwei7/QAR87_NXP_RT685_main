...
#define UacUpBuf_LengthInSamples                (AudioFrameSizeInSamplePerCh * 4)		//when frame size is 128: this is 10.67ms
#define UacUpBuf_MaxReadLengthInSamples			(48)									//USB audio event will read this buffer, and the read size is 1 ms time audio at most (could be 0.5ms, 0.25ms, 0.125ms)

#define UacDnBuf_LengthInMs 					(6 * 2)									//buffer size is 12ms * 48 --- 576 samples, it is 4.5 128sample frame, big enough
...
#define UacDnBuf_CenterLevelInSamples  			(UacDnBuf_LengthInMs*AUDIO_OUT_SAMPLING_RATE_KHZ/2)
