...
void sbc_deconfigured(int err)
{
    ...
    //B36932    CODEC_SetMute(&codec_handle, kCODEC_PlayChannelHeadphoneRight | kCODEC_PlayChannelHeadphoneLeft, true);
...
//       }
        ...
        // Can not start PA after first send data
        if(cnt == 0)
        {
        	return;
        } else if (cnt == 4) { // If I2S comes out later, we can add more delay
