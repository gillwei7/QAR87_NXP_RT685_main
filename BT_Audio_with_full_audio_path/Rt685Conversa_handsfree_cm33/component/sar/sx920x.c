...
int sx920x_poll_event(sx920x_event_t *evt)
{
    ...
    /* 只要 A 或 B 為 BODY 即視為「人體」，但若 startup_any=1，為了保守可以不判定成人體 */
    ...
    if (close_any) {
        out = body_now ? SX920X_EVT_BODY_CLOSE : SX920X_EVT_CLOSE;
        s_body_prev = body_now;  // 記錄當前是否人體
    } else if (far_any) {
        /* 遠離時若上一狀態是人體，輸出「人體遠離」，否則就「遠離」 */
        out = s_body_prev ? SX920X_EVT_BODY_FAR : SX920X_EVT_FAR;
        s_body_prev = false;     // 釋放後當作不是人體
    }
