                                    ...
                                    uint32_t loopTransferCount)
{
    ...
    if (handle->state == (uint32_t)kI2S_DmaStateTx)
    {
        //I2S_TxEnableDMA(base, true);		//			zzz --- really start DMA later outside
    }
    else
    {
        //I2S_RxEnableDMA(base, true);		//			zzz --- really start DMA later outside
    }
    DMA_EnableChannelPeriphRq(handle->dmaHandle->base, handle->dmaHandle->channel);

    /* start transfer */
    //DMA_StartTransfer(handle->dmaHandle);
    DMA_StartTransfer_NoStartImmediately(handle->dmaHandle);	//zzz --- really start DMA later outside
