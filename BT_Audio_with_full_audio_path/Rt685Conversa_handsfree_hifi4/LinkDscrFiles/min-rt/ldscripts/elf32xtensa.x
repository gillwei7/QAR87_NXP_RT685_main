...
_memmap_seg_dsp_core_start = 0x120000;
...
SECTIONS
{
  ...
  .control_reg_addr : ALIGN(4)
  {
    _control_start = ABSOLUTE(.);
    *(.control_reg_addr)
     *(.control_reg_addr.data)
    _control_end = ABSOLUTE(.);
    _memmap_seg_control_end = ALIGN(0x8);
  } >control_seg :control_phdr
  
  NonCacheable.share : ALIGN(4)
  {
    NonCacheableShareWithMcu_start = ABSOLUTE(.);
    . = ALIGN (4);
    *(NonCacheableShareWithMcu)
    . = ALIGN (4);
    NonCacheableSharedWithMcu_end = ABSOLUTE(.);
    _memmap_seg_dsp_uncached_sharedwithmcu_end = ALIGN(0x8);
  } >dsp_uncached_seg_shared :dsp_uncached_phdr
